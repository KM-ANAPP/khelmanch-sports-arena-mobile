...
export default function SportySplashScreen({ onComplete }: { onComplete?: () => void }) {
  ...
  return (
    <AnimatePresence>
      {loading && (
        <motion.div
          className="fixed inset-0 flex flex-col items-center justify-center bg-[#1E2539] z-50"
          initial={{ opacity: 1 }}
          exit={{
            opacity: 0,
        ...
        >
          <div className="relative w-full max-w-md flex flex-col items-center">
            ...
            <div className="mb-8 text-center">
              <motion.h1
                className="text-3xl font-bold text-[#DFE61C] mb-2"
              ...
              <motion.p
                className="text-[#DFE61C]/80 text-sm"
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
            ...
            >
              <motion.div
                className="h-full bg-[#DFE61C] rounded-full"
                initial={{ width: "0%" }}
            ...
            <motion.div
              className="flex items-center text-[#DFE61C]/80 text-sm"
              initial={{ opacity: 0 }}
              animate={{ 
                opacity: [1, 0.5, 1],
                transition: {
            ...
            <div className="absolute inset-0 pointer-events-none">
              {[...Array(5)].map((_, i) => (
                <motion.div
                  key={i}
                  className="absolute rounded-full bg-[#DFE61C]/30"
                  initial={{
                    scale: 0,
                    opacity: 0,
