  ...
  return <Sheet>
      ...
      <SheetContent side="left" className="bg-background">
        ...
        <div className="flex flex-col gap-4 py-4">
          ...
          {isLoggedIn ? <NavItem href="/logout" label="Logout" className="mt-auto text-destructive" /> : <NavItem href="/login" label="Login / Register" className="mt-auto text-secondary" />}
        </div>
      </SheetContent>
    </Sheet>;
}
...
function NavItem({
  ...
  return <Link to={href} className={`px-4 py-2 text-primary hover:bg-muted rounded-md transition-colors ${className || ""}`}>
      {label}
    </Link>;
}
